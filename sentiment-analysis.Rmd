...
In our analysis, we utilized methods based on the approaches found in the tidy-text-mining repository [@robinson2023tidytextmining].
...
```{r tidyflow-ch2, include = FALSE, echo = FALSE, out.width = '100%', fig.cap = "A flowchart of a typical text analysis that uses tidytext for sentiment analysis. This chapter shows how to implement sentiment analysis using tidy data principles."}
...
# Haig Bedros' extension:

## title: "Sentiment Analysis using SentiWordNet"

### Loading the Data:
Here, we're loading the text file `"The Young Castellan: A Tale of the English Civil War"` by `George Manville Fenn`. The data will be read into a tibble and tokenized for analysis.
```{r}
library(tidyverse)
library(tidytext)

gmf_data <- read_lines("https://raw.githubusercontent.com/hbedros/data607_hw10/data/gmf.txt")
...
In this analysis, we'll be using SentiWordNet to assess the sentiment of the text in our corpus. SentiWordNet provides sentiment scores for synsets in WordNet, allowing us to determine the positivity or negativity of words in our text.

### Data Preparation

The SentiWordNet data is also cleaned by retaining essential columns and splitting the terms.
...
### Tokenization and Matching with SentiWordNet

The text corpus is tokenized, and the resulting tokens are matched with the synsets available in the cleaned SentiWordNet data.

```{r}
# Tokenizing the corpus
tokens <- unlist(tokenize_words(corpus))

# Splitting the cleaned_sentiwordnet column and matching with tokens
...
### Sentiment Scoring
For each token, sentiment scores are calculated by averaging the positive and negative scores from SentiWordNet. The net sentiment is the difference between the average positive and negative scores.
...
### Visualization
A visualization to showcase the top positive and negative words based on their net sentiment scores.
...
From the visualization, we can identify the most positive and negative words in our corpus based on their sentiment scores from SentiWordNet.

# Another analysis: MPQA Subjectivity Lexicon

### Loading and Parsing the Lexicon:
Here, we read the MPQA Subjectivity Lexicon file and parse its content into a structured dataframe. We retain relevant columns and rename them for clarity.
...
We match the tokenized words from our text with the words in the subjectivity lexicon.
```{r}
...
Based on the subjectivity lexicon, each word is assigned a sentiment score: +1 for positive words, -1 for negative words, and 0 for neutral or both. The overall sentiment score of the corpus is then computed.
...
A brief summary presenting the overall sentiment score of the corpus.
...
A visualization showing the distribution of sentiment in the corpus based on the subjectivity lexicon.

```{r}
# Create a sentiment label based on priorpolarity
