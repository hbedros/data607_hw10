...
Loading the txt file from:
Title: The Young Castellan: A Tale of the English Civil War
...
Next, we tokenize the corpus and match the tokens with the synsets in SentiWordNet:
...
Now, we can calculate the sentiment scores for our tokens:
...
Finally, let's visualize the top positive and negative words based on their net sentiment scores:
...
## Data Preparation
match the tokens with the subj_clues lexicon:
```{r}
...
## Sentiment Scoring
We'll assign a simple score: +1 for positive words, -1 for negative words, and 0 for neutral/both.
